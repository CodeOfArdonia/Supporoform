...
public class SlimeGrassBlock extends SnowyBlock implements Fertilizable {
    ...
    @Override
    public boolean isValidBonemealTarget(BlockView world, BlockPos pos, BlockState state, boolean isClient) {
     ...
     */
    public static void growGrass(ServerWorld world, Random rand, BlockPos pos, TagKey<Block> validBase, FoliageType foliageType, boolean includeSapling, boolean spread) {
        ...
        for (int i = 0; i < 128; i++) {
            ...
            for (int j = 0; j < i / 16; j++) {
                ...
                if (world.getBlockState(target).isFullCube(world, target)) {
                    continue mainLoop;
                }
                // spread if requested
            ...
            if (world.isAir(target)) {
                ...
                if (plantState.canPlaceAt(world, target)) {
        ...
        if (!isValidPos(state, world, pos)) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (newState != null && canSpread(newState, world, newGrass)) {
                    world.setBlockState(newGrass, newState.with(SNOWY, world.getBlockState(newGrass.up()).isOf(Blocks.SNOW)));
                }
            }
        }
    }
     ...
     */
    private static boolean isValidPos(BlockState targetState, WorldView world, BlockPos pos) {
        ...
        return LayerLightEngine.getLightBlockInto(world, targetState, pos, aboveState, above, Direction.UP, aboveState.getOpacity(world, above)) < world.getMaxLightLevel();
    }
    ...
    private static boolean canSpread(BlockState state, WorldView world, BlockPos pos) {
        BlockPos above = pos.up();
        return isValidPos(state, world, pos) && !world.getFluidState(above).isIn(FluidTags.WATER);
    }
