...
public class ToolStatArgument<T extends IToolStat> implements ArgumentType<T> {
     ...
     */
    public static IToolStat<?> getStat(CommandContext<ServerCommandSource> context, String name) {
        return context.getArgument(name, IToolStat.class);
    }

    @Override
    public T parse(StringReader reader) throws CommandSyntaxException {
        ToolStatId name = new ToolStatId(Identifier.fromCommandInput(reader));
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestIdentifiers(ToolStats.getAllStats().stream()
                .filter(this.filter::isInstance).<Identifier>map(IToolStat::getName)::iterator, builder);
    }
