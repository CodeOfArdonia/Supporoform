...
public class MaterialArgument implements ArgumentType<IMaterial> {
     ...
     */
    public static IMaterial getMaterial(CommandContext<ServerCommandSource> context, String name) {
        return context.getArgument(name, IMaterial.class);
    }

    @Override
    public IMaterial parse(StringReader reader) throws CommandSyntaxException {
        MaterialId name = new MaterialId(Identifier.fromCommandInput(reader));
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestIdentifiers(MaterialRegistry.getMaterials().stream().<Identifier>map(IMaterial::getIdentifier)::iterator, builder);
    }
