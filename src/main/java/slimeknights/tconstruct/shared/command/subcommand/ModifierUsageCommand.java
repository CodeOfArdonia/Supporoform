...
public class ModifierUsageCommand {
    private static final Text SUCCESS = Text.translatable("command.tconstruct.modifier_usage");
    ...
    public static void register(LiteralArgumentBuilder<ServerCommandSource> subCommand) {
        subCommand.requires(sender -> sender.hasPermissionLevel(MantleCommand.PERMISSION_EDIT_SPAWN))
                ...
                // modifier_usage all
                .then(CommandManager.literal("all").executes(context -> runForType(context, ModifierUsages.ALL, null)))
                // modifier_usage recipe [<slot_type>]
                .then(CommandManager.literal("recipe")
                        .then(CommandManager.argument("slot_type", SlotTypeArgument.slotType()).executes(ModifierUsageCommand::runRecipeWithFilter))
                        .executes(context -> runForType(context, ModifierUsages.RECIPE, null)))
                // modifier_usage material_trait
                .then(CommandManager.literal("material_trait").executes(context -> runForType(context, ModifierUsages.MATERIAL_TRAIT, null)))
                // modifier_usage tool_trait
                .then(CommandManager.literal("tool_trait").executes(context -> runForType(context, ModifierUsages.TOOL_TRAIT, null)))
                // modifier_usage unused
                .then(CommandManager.literal("unused").executes(context -> runForType(context, ModifierUsages.UNUSED, null)));
    }

    /**
     * Runs the actual command
     */
    private static int runRecipeWithFilter(CommandContext<ServerCommandSource> context) {
        return runForType(context, ModifierUsages.RECIPE, SlotTypeArgument.getOptional(context, "slot_type"));
    }

    private static int runForType(CommandContext<ServerCommandSource> context, ModifierUsages filter, @Nullable OptionalSlotType slotFilter) {
        ...
        Set<Modifier> toolTraits = RegistryHelper.getTagValueStream(Registries.ITEM, TinkerTags.Items.MODIFIABLE)
