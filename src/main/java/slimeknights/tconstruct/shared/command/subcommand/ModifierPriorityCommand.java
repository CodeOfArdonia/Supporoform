...
public class ModifierPriorityCommand {
    private static final Text SUCCESS = Text.translatable("command.tconstruct.modifier_priority");
    ...
    public static void register(LiteralArgumentBuilder<ServerCommandSource> subCommand) {
        subCommand.requires(sender -> sender.hasPermissionLevel(MantleCommand.PERMISSION_EDIT_SPAWN))
                ...
                .then(CommandManager.argument("modifier_hook", ModifierHookArgument.modifierHook())
     ...
     */
    private static int run(CommandContext<ServerCommandSource> context, boolean filtered) {
