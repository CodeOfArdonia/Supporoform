...
public class ClientGeneratePartTexturesCommand {
    ...
    private static final Text NO_PARTS = TConstruct.makeTranslation("command", "generate_part_textures.no_parts");
    private static final Text NO_MATERIALS = TConstruct.makeTranslation("command", "generate_part_textures.no_materials");
    /**
    ...
    public static void generateTextures(Operation operation, String modId, String materialPath) {
        ...
        if (generatorConfig.sprites.isEmpty()) {
            if (player != null) {
                player.sendMessage(NO_PARTS, false);
            }
            return;
        }
        ...
        if (materialSprites.isEmpty()) {
            if (player != null) {
                player.sendMessage(NO_MATERIALS, false);
            }
            return;
        }

        // prepare the output directory
        Path path = MinecraftClient.getInstance().getResourcePackDir().toAbsolutePath().resolve(PACK_NAME);
        ...
        if (MinecraftClient.getInstance().player != null) {
            MinecraftClient.getInstance().player.sendMessage(Text.translatable(SUCCESS_KEY, count, (deltaTime / 1000000) / 1000f, getOutputComponent(path.toFile())), false);
        }
    }
    ...
    private static void saveImage(Path folder, Identifier location, NativeImage image) {
        Path path = folder.resolve(Paths.get(ResourceType.CLIENT_RESOURCES.getDirectory(),
    ...
    private static GeneratorConfiguration loadGeneratorConfig(ResourceManager manager) {
        ...
        for (String namespace : manager.getAllNamespaces()) {
            Identifier location = new Identifier(namespace, GENERATOR_PART_TEXTURES);
            List<Resource> resources = manager.getAllResources(location);
            if (!resources.isEmpty()) {
                ...
                for (int r = resources.size() - 1; r >= 0; r--) {
                    ...
                    try (BufferedReader reader = resource.getReader()) {
                        JsonObject object = net.minecraft.util.JsonHelper.deserialize(reader);
                        List<PartSpriteInfo> parts = PartSpriteInfo.LIST_LOADABLE.getIfPresent(object, "parts");
                        builder.addAll(parts);
                        if (object.has("overrides")) {
                            for (Entry<String, JsonElement> entry : net.minecraft.util.JsonHelper.getObject(object, "overrides").entrySet()) {
                                String key = entry.getKey();
                                MaterialStatsId statId = MaterialStatsId.PARSER.tryParse(key);
                                if (statId == null) {
                                    TConstruct.LOG.error("Invalid stat ID " + key);
                                } else {
                                    JsonArray array = net.minecraft.util.JsonHelper.asArray(entry.getValue(), key);
                                    for (int i = 0; i < array.size(); i++) {
                                        stats.addVariant(statId, MaterialVariantId.parse(net.minecraft.util.JsonHelper.asString(array.get(i), key + '[' + i + ']')));
                                    }
                                }
                            }
                        }
                        ...
                        if (net.minecraft.util.JsonHelper.getBoolean(object, "replace", false)) {
                            break;
                        }
                    } catch (IOException ex) {
                        log.error("Failed to load modifier models from {} for pack {}", location, resource.getResourcePackName(), ex);
                    }
                }
            }
        }
    ...
    private static List<MaterialSpriteInfo> loadMaterialRenderInfoGenerators(ResourceManager manager, Predicate<MaterialVariantId> validMaterialId) {
        ...
        for (Entry<Identifier, Resource> entry : manager.findResources(MaterialRenderInfoLoader.FOLDER, loc -> loc.getPath().endsWith(".json")).entrySet()) {
