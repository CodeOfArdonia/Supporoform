...
public record IntRange(int min, int max) implements IntPredicate, Loadable<IntRange> {
    ...
    public IntRange convert(JsonElement element, String key) {
        ...
        if (element.isJsonObject()) {
            ...
            int min = JsonHelper.getInt(json, "min", this.min);
            int max = JsonHelper.getInt(json, "max", this.max);
        ...
        int value = JsonHelper.asInt(element, key);
     ...
     */
    @Override
    public IntRange decode(PacketByteBuf buffer) {
     ...
     * Reads a range from the buffer
     */
    public static IntRange fromNetwork(PacketByteBuf buffer) {
    ...
    @Override
    public void encode(PacketByteBuf buffer, IntRange object) {
