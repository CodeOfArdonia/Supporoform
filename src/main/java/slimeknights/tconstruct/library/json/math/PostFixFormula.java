...
public record PostFixFormula(List<StackOperation> operations, int numArguments) implements ModifierFormula {
    ...
    public static PostFixFormula deserialize(JsonObject json, String[] variableNames) {
        ...
        return new PostFixFormula(JsonHelper.parseList(json, "formula", (element, key) -> {
            ...
            throw new JsonSyntaxException("Expected " + key + " to be a string or number, was " + net.minecraft.util.JsonHelper.getType(element));
     ...
     */
    public static PostFixFormula fromNetwork(PacketByteBuf buffer, int numArguments) {
        return fromNetwork(buffer, buffer.readShort(), numArguments);
    }

    /**
     * Common logic between {@link #fromNetwork(PacketByteBuf, int)} and {@link ModifierFormula#fromNetwork(PacketByteBuf, int, FallbackFormula)}
     */
    static PostFixFormula fromNetwork(PacketByteBuf buffer, short size, int numArguments) {
