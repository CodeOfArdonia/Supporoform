...
public sealed interface MaterialVariantId permits MaterialId, MaterialVariantIdImpl {
    ...
    static MaterialVariantId create(MaterialId id, String variant) {
        ...
        if (!Identifier.isPathValid(variant)) {
            throw new InvalidIdentifierException("Non [a-z0-9/._-] character in variant of material variant ID: " + id + "#" + variant);
        }
    ...
    static MaterialVariantId fromJson(JsonObject json, String key) {
        String text = JsonHelper.getString(json, key);
    ...
    static MaterialVariantId convertJson(JsonElement json, String key) {
        String text = JsonHelper.asString(json, key);
        return parse(text, key);
    }
     ...
     */
    default void toNetwork(PacketByteBuf buf) {
     ...
     */
    static MaterialVariantId fromNetwork(PacketByteBuf buf) {
        return parse(buf.readString(Short.MAX_VALUE));
    }
}
