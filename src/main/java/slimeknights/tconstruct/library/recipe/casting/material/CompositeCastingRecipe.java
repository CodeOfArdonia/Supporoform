...
public class CompositeCastingRecipe extends MaterialCastingRecipe {
    ...
    public CompositeCastingRecipe(TypeAwareRecipeSerializer<?> serializer, Identifier id, String group, IMaterialItem result, int itemCost, @Nullable MaterialStatsId castingStatConflict) {
        super(serializer, id, group, Ingredient.ofItems(result), itemCost, result, true, false);
                ...
                if (!output.isUnknown() && input != null && !input.isUnknown()
                    ...
                    if (!fluids.isEmpty()) {
                        ...
                        recipes.add(new DisplayCastingRecipe(type, List.of(result.withMaterial(input.getVariant())), fluids, result.withMaterial(output.getVariant()),
                                ICastingRecipe.calcCoolingTime(recipe.getTemperature(), itemCost * fluids.stream().mapToLong(FluidStack::getAmount).max().orElse(0)),
                                isConsumed()));
                    }
                }
            }
