...
public abstract class AbstractModifierRecipeBuilder<T extends AbstractModifierRecipeBuilder<T>> extends AbstractRecipeBuilder<T> {
    ...
    protected Ingredient tools = Ingredient.fromTag(TinkerTags.Items.MODIFIABLE);
     ...
     * @param id       Recipe ID
     */
    public T saveSalvage(Consumer<RecipeJsonProvider> consumer, Identifier id) {
    ...
    protected ModifierSalvage makeSalvage(Identifier id) {
        return new ModifierSalvage(id, this.tools, this.maxToolSize, this.result, VALID_LEVEL.range(this.minLevel, this.useSalvageMax ? this.maxLevel : VALID_LEVEL.max()), this.slots);
    }
}
