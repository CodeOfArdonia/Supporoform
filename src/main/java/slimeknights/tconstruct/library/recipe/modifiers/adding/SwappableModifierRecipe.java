...
public class SwappableModifierRecipe extends ModifierRecipe {
    ...
    /**
     * @deprecated use {@link #SwappableModifierRecipe(Identifier, List, Ingredient, int, ModifierId, String, VariantFormatter, SlotCount, boolean)}}
     */
    @Deprecated
    public SwappableModifierRecipe(Identifier id, List<SizedIngredient> inputs, Ingredient toolRequirement, int maxToolSize, ModifierId result, String value, @Nullable SlotCount slots, boolean allowCrystal) {
    ...
    public RecipeResult<ItemStack> getValidatedResult(ITinkerStationContainer inv, DynamicRegistryManager manager) {
        ...
        if (tool.getUpgrades().getLevel(modifier) == 0) {
            needsModifier = true;
            Text commonError = validatePrerequisites(tool);
            if (commonError != null) {
    ...
    public interface VariantFormatter {
         ...
         */
        Text format(ModifierId modifier, String variant);
         ...
         */
        VariantFormatter DEFAULT = LOADER.register(getResource("default"), (modifier, variant) -> Text.translatable(Util.makeTranslationKey("modifier", modifier) + "." + variant));
        /**
