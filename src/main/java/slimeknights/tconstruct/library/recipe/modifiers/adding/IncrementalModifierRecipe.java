...
public class IncrementalModifierRecipe extends AbstractModifierRecipe {
    ...
    @Override
    public boolean matches(ITinkerStationContainer inv, World level) {
    ...
    public RecipeResult<ItemStack> getValidatedResult(ITinkerStationContainer inv, DynamicRegistryManager manager) {
        ...
        if (crystal || newLevel) {
            Text commonError = this.validatePrerequisites(tool);
            if (commonError != null) {
        ...
        if (crystal) {
            ...
            tool.addModifierAmount(modifier, getAvailableAmount(inv, this.input, this.amountPerInput), this.neededPerLevel);
        }
    ...
    public void updateInputs(ItemStack result, IMutableTinkerStationContainer inv, boolean isServer) {
        ...
        if (inputNeed == 0 || inputNeed == this.neededPerLevel) {
            ...
            updateInputs(inv, this.input, resultEntry.getAmount(this.neededPerLevel) - inputEntry.getAmount(0), this.amountPerInput, this.leftover.get());
        } else {
            // so the sizes mismatch, need to rescale the input, the result, and the amount consumed per item
            int gcd = IntMath.gcd(inputNeed, this.neededPerLevel);
            ...
            updateInputs(inv, this.input, (used + recipeScale - 1) / recipeScale, this.amountPerInput, this.leftover.get());
        }
    }
    ...
    private List<List<ItemStack>> getInputs() {
        if (this.slotCache == null) {
            ...
            List<ItemStack> items = Arrays.asList(this.input.getMatchingStacks());
    ...
    /**
     * @deprecated use {@link JsonUtils#getAsItemStack(JsonObject, String)}
     */
    @Deprecated
