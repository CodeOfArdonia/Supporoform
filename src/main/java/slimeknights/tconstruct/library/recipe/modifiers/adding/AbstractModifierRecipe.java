...
public abstract class AbstractModifierRecipe implements ITinkerStationRecipe, IDisplayModifierRecipe {
    ...
    @Override
    public abstract RecipeResult<ItemStack> getValidatedResult(ITinkerStationContainer inv, DynamicRegistryManager manager);
    ...
    public static boolean matchesCrystal(ITinkerStationContainer container, ModifierId match) {
        ...
        for (int i = 0; i < container.getInputCount(); i++) {
            ...
            if (!stack.isEmpty()) {
                ...
                // must be a crystal
                if (found || !stack.isOf(TinkerModifiers.modifierCrystal.asItem())) {
                    return false;
                }
     ...
     */
    @Nullable
    protected Text validateLevel(int resultLevel) {
        if (resultLevel < this.level.min()) {
            return Text.translatable(checkTraitLevel ? KEY_MIN_LEVEL_TRAITS : KEY_MIN_LEVEL, result.get().getDisplayName(this.level.min() - 1));
        }
        // max level of modifier
        if (resultLevel > this.level.max()) {
            return Text.translatable(checkTraitLevel ? KEY_MAX_LEVEL_TRAITS : KEY_MAX_LEVEL, result.get().getDisplayName(), this.level.max());
        }
        return null;
    }
     ...
     */
    @Nullable
    protected static Text checkSlots(IToolStackView tool, @Nullable SlotCount slots) {
        if (slots != null) {
            int count = slots.count();
            if (tool.getFreeSlots(slots.type()) < count) {
                if (count == 1) {
                    return Text.translatable(KEY_NOT_ENOUGH_SLOT, slots.type().getDisplayName());
                } else {
                    return Text.translatable(KEY_NOT_ENOUGH_SLOTS, count, slots.type().getDisplayName());
                }
            }
        }
        return null;
     ...
     */
    @Nullable
    protected Text validatePrerequisites(IToolStackView tool, int resultLevel) {
        Text error = validateLevel(resultLevel);
        if (error != null) {
            return error;
        }
     ...
     */
    @Nullable
    protected Text validatePrerequisites(IToolStackView tool) {
