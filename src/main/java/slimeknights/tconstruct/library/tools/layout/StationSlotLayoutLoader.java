 ...
 */
@Log4j2
public class StationSlotLayoutLoader extends JsonDataLoader {
    ...
    @Override
    protected void apply(Map<Identifier, JsonElement> splashList, ResourceManager resourceManager, Profiler profiler) {
        ...
        for (Entry<Identifier, JsonElement> entry : splashList.entrySet()) {
            ...
            try {
                ...
                JsonObject object = JsonHelper.asObject(value, "station_layout");
        ...
        List<String> missing = this.requiredLayouts.stream().filter(name -> !this.layoutMap.containsKey(name)).map(Identifier::toString).collect(Collectors.toList());
        if (!missing.isEmpty()) {
