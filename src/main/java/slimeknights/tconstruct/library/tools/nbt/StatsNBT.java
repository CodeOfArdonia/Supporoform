...
public class StatsNBT {
    ...
    @SuppressWarnings("unchecked")
    @Nullable
    private static <T> NbtElement serialize(IToolStat<T> stat, Object value) {
    ...
    public NbtCompound serializeToNBT() {
        ...
        for (Entry<IToolStat<?>, Object> entry : stats.entrySet()) {
            ...
            NbtElement serialized = serialize(stat, entry.getValue());
            if (serialized != null) {
     ...
     */
    @SuppressWarnings("unchecked")
    private static <T> void toNetwork(PacketByteBuf buffer, IToolStat<T> stat, Object value) {
    ...
    public void toNetwork(PacketByteBuf buffer) {
        ...
        for (Entry<IToolStat<?>, Object> entry : stats.entrySet()) {
            IToolStat<?> stat = entry.getKey();
            buffer.writeString(stat.getName().toString());
     ...
     */
    public static StatsNBT fromNetwork(PacketByteBuf buffer) {
        ImmutableMap.Builder<IToolStat<?>, Object> builder = ImmutableMap.builder();
    ...
    public static final RecordLoadable<StatsNBT> LOADABLE = new RecordLoadable<>() {
        ...
        @Override
        public StatsNBT decode(PacketByteBuf buffer, TypedMap context) {
            return StatsNBT.fromNetwork(buffer);
        }

        @Override
        public void encode(PacketByteBuf buffer, StatsNBT stats) {
