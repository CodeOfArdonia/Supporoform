...
public class ToolStack implements IToolStackView {
    ...
    public ModDataNBT getPersistentData() {
        if (persistentModData == null) {
            ...
            if (nbt.contains(TAG_PERSISTENT_MOD_DATA, NbtElement.COMPOUND_TYPE)) {
    ...
    public IModDataView getVolatileData() {
        if (volatileModData == null) {
            // parse if the tag already exists
            if (nbt.contains(TAG_VOLATILE_MOD_DATA, NbtElement.COMPOUND_TYPE)) {
    ...
    public void ensureHasData() {
        ...
        if (definition.isDataLoaded()) {
            ...
            boolean needsMaterials = definition.hasMaterials() && !nbt.contains(TAG_MATERIALS, NbtElement.LIST_TYPE);
            if (needsMaterials || !isInitialized(nbt)) {
                // randomize materials if missing
                if (needsMaterials) {
                    setMaterialsRaw(definition.getHook(ToolHooks.MISSING_MATERIALS).fillMaterials(definition, Random.create()));
                }
                rebuildStats();
            }
        }
     ...
     * @return True if initialized
     */
    public static boolean isInitialized(NbtCompound tag) {
        return tag.contains(TAG_STATS, NbtElement.COMPOUND_TYPE);
    }
    ...
    public static boolean hasMaterials(ItemStack stack) {
        NbtCompound nbt = stack.getNbt();
        return nbt != null && nbt.contains(TAG_MATERIALS, NbtElement.LIST_TYPE);
    }
    ...
    /**
     * Ensures the given item stack is initialized. Intended to be called in {@link Item#onCraft(ItemStack, World, PlayerEntity)}
     *
     * @param stack          ItemStack to initialize
     ...
     * @param definition Tool definition
     */
    public static void verifyTag(Item item, NbtCompound tag, ToolDefinition definition) {
        ...
        boolean hasMaterials = tag.contains(ToolStack.TAG_MATERIALS, NbtElement.LIST_TYPE);
        if (hasMaterials) {
            MaterialIdNBT stored = MaterialIdNBT.readFromNBT(tag.getList(ToolStack.TAG_MATERIALS, NbtElement.STRING_TYPE));
