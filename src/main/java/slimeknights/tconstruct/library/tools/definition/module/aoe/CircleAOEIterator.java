...
public record CircleAOEIterator(int diameter, boolean is3D) implements AreaOfEffectIterator.Loadable {
     ...
     */
    public static Iterable<BlockPos> calculate(IToolStackView tool, ItemStack stack, World world, PlayerEntity player, BlockPos origin, Direction sideHit, int diameter, boolean is3D, AOEMatchType matchType) {
        // skip if no work
        if (diameter == 1) {
