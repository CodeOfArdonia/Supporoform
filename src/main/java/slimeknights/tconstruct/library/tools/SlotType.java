...
public final class SlotType {
     ...
     */
    public static SlotType read(PacketByteBuf buffer) {
    ...
    public Text getDisplayName() {
        if (this.displayName == null) {
            this.displayName = Text.translatable(KEY_DISPLAY + this.name);
        }
    ...
    public record SlotCount(SlotType type, int count) {
        public static final Loadable<SlotCount> LOADABLE = new Loadable<>() {
            ...
            @Override
            public SlotCount decode(PacketByteBuf buffer) {
                return new SlotCount(SlotType.read(buffer), buffer.readVarInt());
            }

            @Override
            public void encode(PacketByteBuf buffer, SlotCount slots) {
        ...
        private record NullableSlotCountField<P>(String key,
            ...
            public SlotCount get(JsonObject json) {
                return LOADABLE.getOrDefault(json, this.key, null);
            }
            ...
            public void serialize(P parent, JsonObject json) {
                ...
                if (count != null) {
                    json.add(this.key, LOADABLE.serialize(count));
                }
            }
