...
public class ToolInventoryCapability extends InventoryModifierHookIterator<ModifierEntry> implements IItemHandlerModifiable {
    ...
    public static boolean isBlacklisted(ItemStack stack) {
        return !stack.getItem().canBeNested() || stack.isIn(TinkerTags.Items.TOOL_INVENTORY_BLACKLIST) || stack.getCapability(ForgeCapabilities.ITEM_HANDLER).isPresent();
    }
    ...
    public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
        ...
        if (current.isEmpty()) {
            int canInsert = Math.min(stack.getCount(), Math.min(stack.getMaxCount(), slotLimit));
            ...
            int limit = Math.min(current.getMaxCount(), slotLimit);
     ...
     */
    public static ActionResult tryOpenContainer(ItemStack stack, IToolStackView tool, PlayerEntity player, EquipmentSlot slotType) {
     ...
     * Opens the tool inventory container if an inventory is present on the given tool
     */
    public static ActionResult tryOpenContainer(ItemStack stack, @Nullable IToolStackView tool, ToolDefinition definition, PlayerEntity player, EquipmentSlot slotType) {
        ...
        if (handler != null) {
            if (player instanceof ServerPlayerEntity serverPlayer) {
                NetworkHooks.openScreen(serverPlayer, new SimpleNamedScreenHandlerFactory(
