...
abstract class FluidModifierHookIterator<I> extends CompoundIndexHookIterator<FluidModifierHook, I> {
    ...
    protected int fill(IToolStackView tool, FluidStack resource, FluidAction action) {
        ...
        while (iterator.hasNext()) {
            ...
            int filled = this.getHook(iterator.next()).fill(tool, this.indexEntry, resource, action);
            if (filled > 0) {
    ...
    public FluidStack drain(IToolStackView tool, FluidStack resource, FluidAction action) {
        ...
        while (iterator.hasNext()) {
            ...
            FluidStack drained = this.getHook(iterator.next()).drain(tool, this.indexEntry, resource, action);
            if (!drained.isEmpty()) {
    ...
    public FluidStack drain(IToolStackView tool, int maxDrain, FluidAction action) {
        ...
        while (iterator.hasNext()) {
            ...
            if (toDrain.isEmpty()) {
                FluidStack drained = this.getHook(next).drain(tool, this.indexEntry, maxDrain, action);
                if (!drained.isEmpty()) {
                ...
                FluidStack drained = this.getHook(next).drain(tool, this.indexEntry, toDrain, action);
                if (!drained.isEmpty()) {
                    drainedSoFar.grow(drained.getAmount());
