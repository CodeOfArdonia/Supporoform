...
public class TooltipBuilder {
     ...
     */
    private static final Text TOOLTIP_BROKEN = TConstruct.makeTranslation("tooltip", "tool.broken").formatted(Formatting.BOLD, Formatting.DARK_RED);
    /**
     * Prefixed broken string
     */
    private static final Text TOOLTIP_BROKEN_PREFIXED = ToolStats.DURABILITY.getPrefix().append(TOOLTIP_BROKEN);
     ...
     */
    private <T> Text formatValue(IToolStat<T> stat) {
    ...
    public static Text formatPartialAmount(int value, int max) {
        return Text.literal(Util.COMMA_FORMAT.format(value))
                .styled(style -> style.withColor(valueToColor(value, max)))
                .append(Text.literal(" / ").formatted(Formatting.GRAY))
                .append(Text.literal(Util.COMMA_FORMAT.format(max)).styled(APPLY_MAX));
    }
     ...
     * @return the tooltip builder
     */
    public TooltipBuilder addWithAttribute(INumericToolStat<?> stat, EntityAttribute attribute) {
    ...
    public TooltipBuilder addModifierInfo(boolean advanced) {
        for (ModifierEntry entry : this.tool.getModifierList()) {
            if (entry.getModifier().shouldDisplay(advanced)) {
                this.tooltips.add(entry.getModifier().getDisplayName(this.tool, entry));
            }
        }
        return this;
    }
