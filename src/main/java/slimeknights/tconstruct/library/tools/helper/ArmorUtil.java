...
public class ArmorUtil {
    ...
    /**
     * Inverse of {@link net.minecraft.entity.DamageUtil#getDamageLeft(float, float, float)}  with respect to damage
     *
    ...
    /**
     * Extension of of {@link net.minecraft.entity.DamageUtil#getInflictedDamage(float, float)} to allow increasing damage via negative numbers
     *
    ...
    /**
     * Extension of of {@link net.minecraft.entity.DamageUtil#getInflictedDamage(float, float)} to allow increasing damage via negative numbers and a higher cap
     *
     * @param damage           Damage to absorb (or increase)
     * @param enchantModifiers Enchantment modifier amount, between -20 and 20
     * @return Original damage to be dealt
    ...
    public static float getDamageAfterMagicAbsorb(float damage, float enchantModifiers, float cap) {
        ...
        return damage * (1f - MathHelper.clamp(enchantModifiers, -20f, cap) / 25f);
    }

    /**
     * Inverse of {@link net.minecraft.entity.DamageUtil#getInflictedDamage(float, float)} with respect to damage
     *
     * @param damage           Damage returned by the vanilla function, must be 0 or more
    ...
    /**
     * Calculates the final damage for use in LivingHurtEvent. Requires applying several inverse functions to cancel out vanilla formulas that are applied later
     *
    ...
    public static float getDamageForEvent(float originalDamage, float armor, float toughness, float vanillaModifiers, float finalModifiers, float modifierCap) {
        ...
        if (armor > 0) {
            damage = getDamageLeft(damage, armor, toughness);
        }
