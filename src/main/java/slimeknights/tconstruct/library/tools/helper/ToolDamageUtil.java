...
public class ToolDamageUtil {
    ...
    public static void breakTool(ItemStack stack) {
        stack.getOrCreateNbt().putBoolean(ToolStack.TAG_BROKEN, true);
    }
     ...
     * @return true if the tool broke when damaging
     */
    public static boolean damageAnimated(IToolStackView tool, int amount, LivingEntity entity, Hand hand) {
        if (damage(tool, amount, entity, entity.getStackInHand(hand))) {
    ...
    public static boolean damageAnimated(IToolStackView tool, int amount, LivingEntity entity) {
        return damageAnimated(tool, amount, entity, entity.isUsingItem() ? entity.getActiveHand() : Hand.MAIN_HAND);
    }

    /**
     * Implements {@link net.minecraft.item.Item#damageItem(ItemStack, int, LivingEntity, Consumer)} for a modifiable item
     */
    public static <T extends LivingEntity> void handleDamageItem(ItemStack stack, int amount, T damager, Consumer<T> onBroken) {
        ...
        if (stack.getItem().isDamageable() && ToolDamageUtil.damage(ToolStack.from(stack), amount, damager, stack)) {
