...
public class FixedArmorTextureSupplier implements ArmorTextureSupplier {
     ...
     */
    public static ArmorTexture getTexture(Identifier base, String suffix, int color) {
        Identifier name = LocationExtender.INSTANCE.suffix(base, suffix);
    ...
    public ArmorTexture getArmorTexture(ItemStack stack, TextureType textureType) {
        ArmorTexture texture = this.textures[textureType.ordinal()];
        // skip the modifier check if the texture is not set, saves some effort
        if (texture != ArmorTexture.EMPTY && (this.modifier == null || ModifierUtil.getModifierLevel(stack, this.modifier) > 0)) {
            return texture;
        }
        return ArmorTexture.EMPTY;
    }
