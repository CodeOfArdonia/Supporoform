...
public class ModifierModelManager implements IEarlySafeManagerReloadListener {
                ...
                if (name == null) {
                    ...
                    if (!models.containsKey(name)) {
                        ...
                        if (element.isJsonNull()) {
                            ...
                            IUnbakedModifierModel model = getLoader(key, net.minecraft.util.JsonHelper.getString(object, "type"));
     ...
     */
    private static SpriteIdentifier getModifierTexture(Identifier modifierRoot, Identifier modifierId, String suffix) {
        return ForgeHooksClient.getBlockMaterial(new Identifier(modifierRoot.getNamespace(), modifierRoot.getPath() + modifierId.getNamespace() + "_" + modifierId.getPath() + suffix));
    }
     ...
     */
    @Nullable
    private static SpriteIdentifier getTexture(List<Identifier> modifierRoots, Predicate<SpriteIdentifier> textureAdder, Identifier modifier, String suffix) {
        ...
        for (Identifier root : modifierRoots) {
            SpriteIdentifier texture = getModifierTexture(root, modifier, suffix);
     ...
     */
    public static Map<ModifierId, IBakedModifierModel> getModelsForTool(List<Identifier> smallModifierRoots, List<Identifier> largeModifierRoots, Collection<SpriteIdentifier> textures) {
        ...
        Predicate<SpriteIdentifier> textureAdder = DynamicTextureLoader.getTextureAdder(textures, Config.CLIENT.logMissingModifierTextures.get());
