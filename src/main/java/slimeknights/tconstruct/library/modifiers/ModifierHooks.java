...
public class ModifierHooks {
     ...
     * Note the charged interaction hooks will only fire for the modifier that called {@link GeneralInteractionModifierHook#startUsing(IToolStackView, ModifierId, LivingEntity, Hand)},
     * meaning there is no need to manually track that you were called.
     */
    public static final ModuleHook<GeneralInteractionModifierHook> GENERAL_INTERACT = register("general_interact", GeneralInteractionModifierHook.class, GeneralInteractionModifierHook.FirstMerger::new, ((tool, modifier, player, hand, source) -> ActionResult.PASS));
    /**
    ...
    /**
     * @deprecated worse parameter order, use {{@link #register(Identifier, Class, Function, Object)}}
     */
    @Deprecated(forRemoval = true, since = "3.8.1")
    public static <T> ModuleHook<T> register(Identifier name, Class<T> filter, T defaultInstance, @Nullable Function<Collection<T>, T> merger) {
