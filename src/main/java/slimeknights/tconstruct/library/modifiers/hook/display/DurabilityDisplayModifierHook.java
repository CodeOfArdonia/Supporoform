...
public interface DurabilityDisplayModifierHook {
    ...
    static boolean showDurabilityBar(ItemStack stack) {
        ...
        if (!stack.getItem().isDamageable() || !stack.isIn(TinkerTags.Items.DURABILITY)) {
            return false;
        }
        ...
        return tool.getItem().isDamageable() && tool.getDamage() > 0;
    }
    ...
    static int getDurabilityRGB(ItemStack stack) {
        ...
        return MathHelper.hsvToRgb(Math.max(0.0f, (max - tool.getDamage()) / max / 3.0f), 1.0f, 1.0f);
    }
