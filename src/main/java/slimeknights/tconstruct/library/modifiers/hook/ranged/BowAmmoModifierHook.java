...
public interface BowAmmoModifierHook {
     ...
     */
    static boolean hasAmmo(IToolStackView tool, ItemStack bowStack, PlayerEntity player, Predicate<ItemStack> predicate) {
        ...
        if (!player.getProjectileType(bowStack).isEmpty()) {
            return true;
        }
     ...
     * @return Found ammo
     */
    static ItemStack findAmmo(IToolStackView tool, ItemStack bow, PlayerEntity player, Predicate<ItemStack> predicate) {
        ...
        boolean creative = player.getAbilities().creativeMode || player.getWorld().isClient;

        // first search, find what ammo type we want
        ItemStack standardAmmo = player.getProjectileType(bow);
        ...
        if (resultStack.isEmpty()) {
            ...
            if (standardAmmo.isEmpty()) {
                player.getInventory().removeOne(standardAmmo);
            }
        }
        ...
        if (resultStack.getCount() >= projectilesDesired || player.getWorld().isClient) {
            return resultStack;
        }

        // not enough? keep searching until we fill the stack
        ItemStack match = resultStack;
        predicate = stack -> ItemStack.canCombine(stack, match);
        hasEnough:
        do {
            ...
            if (needed <= standardAmmo.getCount()) {
                ...
                resultStack.increment(standardAmmo.getCount());
                player.getInventory().removeOne(standardAmmo);
            } else {
                // found what we need, we are done
                standardAmmo.decrement(needed);
                resultStack.increment(needed);
                break;
            }
