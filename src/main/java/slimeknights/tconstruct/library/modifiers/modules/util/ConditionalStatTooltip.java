...
public interface ConditionalStatTooltip extends TooltipModifierHook, ConditionalModule<IToolStackView> {
     ...
     */
    float computeTooltipValue(IToolStackView tool, ModifierEntry entry, @Nullable PlayerEntity player);

    @Override
    default void addTooltip(IToolStackView tool, ModifierEntry entry, @Nullable PlayerEntity player, List<Text> tooltip, TooltipKey tooltipKey, TooltipContext tooltipFlag) {
        ...
        if (stat.supports(tool.getItem()) && condition().matches(tool, entry) && TinkerPredicate.matchesInTooltip(holder, player, tooltipKey)) {
            ...
            Text statName = TooltipModifierHook.statName(modifier, stat);
