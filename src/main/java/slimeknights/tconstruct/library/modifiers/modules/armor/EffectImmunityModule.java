 ...
 */
public record EffectImmunityModule(StatusEffect effect,
    ...
    public static final ComputableDataKey<Multiset<StatusEffect>> EFFECT_IMMUNITY = TConstruct.createKey("effect_immunity", HashMultiset::create);
    ...
    public EffectImmunityModule(StatusEffect effect) {
    ...
    public void onEquip(IToolStackView tool, ModifierEntry modifier, EquipmentChangeContext context) {
        if (!tool.isBroken() && ArmorLevelModule.validSlot(tool, context.getChangedSlot(), TinkerTags.Items.HELD_ARMOR) && this.condition.matches(tool, modifier)) {
            context.getTinkerData().ifPresent(data -> data.computeIfAbsent(EFFECT_IMMUNITY).add(this.effect));
        }
    }
