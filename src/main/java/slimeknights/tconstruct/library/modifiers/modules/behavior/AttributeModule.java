 ...
 */
public record AttributeModule(String unique, EntityAttribute attribute, Operation operation, ToolFormula formula,
                              UUID[] slotUUIDs,
    ...
    public static UUID[] slotsToUUIDs(String name, Collection<EquipmentSlot> slots) {
        ...
        for (EquipmentSlot slot : slots) {
            slotUUIDs[slot.getArmorStandSlotId()] = getUUID(name, slot);
        }
        return slotUUIDs;
    }
    ...
    public static Set<EquipmentSlot> uuidsToSlots(UUID[] uuids) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            if (uuids[slot.getArmorStandSlotId()] != null) {
                set.add(slot);
            }
        }
        return set;
    ...
    public void addAttributes(IToolStackView tool, ModifierEntry modifier, EquipmentSlot slot, BiConsumer<EntityAttribute, EntityAttributeModifier> consumer) {
        if (condition.matches(tool, modifier)) {
            UUID uuid = slotUUIDs[slot.getArmorStandSlotId()];
            if (uuid != null) {
                consumer.accept(attribute, new EntityAttributeModifier(uuid, unique + "." + slot.getName(), formula.apply(tool, modifier), operation));
            }
        }
    }
    ...
    public static class Builder extends VariableFormula.Builder<Builder, AttributeModule, ToolVariable> {
        ...
        protected Builder(EntityAttribute attribute, Operation operation) {
            super(VARIABLES);
