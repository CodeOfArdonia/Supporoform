 ...
 */
public record FluidEffects(FluidIngredient ingredient, List<FluidEffect<? super Block>> blockEffects,
                           List<FluidEffect<? super Entity>> entityEffects) {
    ...
    private <C extends FluidEffectContext> int apply(FluidStack fluid, float level, C context, List<FluidEffect<? super C>> effects, FluidAction action) {
        long amountPerLevel = this.getAmount(fluid.getFluid());
        float scale;
