...
public record RestoreHungerFluidEffect(int hunger, float saturation, boolean canAlwaysEat,
                                       ItemOutput representative) implements FluidEffect<Entity> {
    ...
    public float apply(FluidStack fluid, EffectLevel level, Entity context, FluidAction action) {
        ...
        if (target instanceof PlayerEntity player && player.canConsume(this.canAlwaysEat)) {
            // we always consume the full amount as while hunger is capped, saturation is not
            float value = level.value();
            if (action.execute()) {
                int finalHunger = (int) (this.hunger * value);
                player.getHungerManager().add(finalHunger, this.saturation);
