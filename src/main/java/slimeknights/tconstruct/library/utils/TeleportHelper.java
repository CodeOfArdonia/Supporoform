...
public class TeleportHelper {
    ...
    public static boolean randomNearbyTeleport(LivingEntity living, ITeleportEventFactory factory) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            double y = MathHelper.clamp(posY + (double) (living.getRandom().nextInt(16) - 8), 0.0D, living.getEntityWorld().getHeight() - 1);
            ...
            if (!event.isCanceled() && living.teleport(event.getTargetX(), event.getTargetY(), event.getTargetZ(), true)) {
                SoundEvent soundevent = Sounds.SLIME_TELEPORT.getSound();
                living.getWorld().playSound(null, posX, posY, posZ, soundevent, SoundCategory.PLAYERS, 1.0F, 1.0F);
