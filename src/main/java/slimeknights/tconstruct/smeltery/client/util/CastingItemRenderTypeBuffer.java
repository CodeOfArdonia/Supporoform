...
import io.github.fabricators_of_create.porting_lib.mixin.accessors.client.accessor.TextureStateShardAccessor;
...
public class CastingItemRenderTypeBuffer implements VertexConsumerProvider {
    ...
    public VertexConsumer getBuffer(RenderLayer type) {
        if (this.alpha < 255 && MAKE_TRANSPARENT.contains(type.name) && type instanceof RenderLayer.MultiPhase composite && composite.phases.texture instanceof RenderPhase.Texture textureState) {
            Identifier texture = textureState.id.orElse(PlayerScreenHandler.BLOCK_ATLAS_TEXTURE);
