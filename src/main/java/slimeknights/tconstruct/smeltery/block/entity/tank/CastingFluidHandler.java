...
public class CastingFluidHandler implements IFluidHandler {
    ...
    public void readFromTag(NbtCompound nbt) {
        ...
        if (nbt.contains(TAG_FLUID, NbtElement.COMPOUND_TYPE)) {
            setFluid(FluidStack.loadFluidStackFromNBT(nbt.getCompound(TAG_FLUID)));
        }
        if (nbt.contains(TAG_FILTER, NbtElement.STRING_TYPE)) {
    ...
    public NbtCompound writeToTag(NbtCompound nbt) {
        ...
        if (!this.fluid.isEmpty()) {
            nbt.put(TAG_FLUID, this.fluid.writeToNBT(new NbtCompound()));
        }
        if (this.filter != Fluids.EMPTY) {
            nbt.putString(TAG_FILTER, Registry.FLUID.getKey(this.filter).toString());
        }
        return nbt;
    }
}
