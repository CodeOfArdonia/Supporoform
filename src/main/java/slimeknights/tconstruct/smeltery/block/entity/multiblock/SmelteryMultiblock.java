...
public class SmelteryMultiblock extends HeatingStructureMultiblock<SmelteryBlockEntity> {
    ...
    protected boolean isValidBlock(Block block) {
        return block.getRegistryEntry().isIn(TinkerTags.Blocks.SMELTERY);
    }

    @Override
    protected boolean isValidFloor(Block block) {
        return block.getRegistryEntry().isIn(TinkerTags.Blocks.SMELTERY_FLOOR);
    }

    @Override
    protected boolean isValidTank(Block block) {
        return block.getRegistryEntry().isIn(TinkerTags.Blocks.SMELTERY_TANKS);
    }

    @Override
    protected boolean isValidWall(Block block) {
        return block.getRegistryEntry().isIn(TinkerTags.Blocks.SMELTERY_WALL);
    }
}
