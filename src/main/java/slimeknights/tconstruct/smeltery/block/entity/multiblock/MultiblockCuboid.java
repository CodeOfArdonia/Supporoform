...
public abstract class MultiblockCuboid<T extends MultiblockStructureData> {
     ...
     */
    protected static final Text INVALID_INNER_BLOCK = TConstruct.makeTranslation("multiblock", "generic.invalid_inner_block");
    /**
     * Error if the structure inside is not valid
     */
     ...
     */
    protected static final Text INVALID_FLOOR_BLOCK = TConstruct.makeTranslation("multiblock", "generic.invalid_floor_block");
    /**
     * Error if a block is invalid in the ceiling
     */
    protected static final Text INVALID_CEILING_BLOCK = TConstruct.makeTranslation("multiblock", "generic.invalid_floor_block");
    /**
     * Error if a block is invalid in the walls
     */
    protected static final Text INVALID_WALL_BLOCK = TConstruct.makeTranslation("multiblock", "generic.invalid_wall_block");
    /**
     * Error if the structure floor has no frame
     */
    protected static final Text INVALID_FLOOR_FRAME = TConstruct.makeTranslation("multiblock", "generic.invalid_floor_frame");
    /**
     * Error if the structure ceiling has no frame
     */
    protected static final Text INVALID_CEILING_FRAME = TConstruct.makeTranslation("multiblock", "generic.invalid_ceiling_frame");
    /**
     * Error if the structure wall has no frame
     */
    protected static final Text INVALID_WALL_FRAME = TConstruct.makeTranslation("multiblock", "generic.invalid_wall_frame");
    ...
    protected BlockPos getOuterPos(World world, BlockPos pos, Direction direction, int limit) {
        for (int i = 0; i < limit && world.canSetBlock(pos) && isInnerBlock(world, pos); i++) {
        ...
        if (hasFrame) {
            ...
            for (int x = from.getX(); x <= to.getX(); x++) {
                if (!frameCheck.test(mutable.set(x, height, from.getZ())))
                    return MultiblockResult.error(mutable.toImmutable(), frameError);
    ...
    protected MultiblockResult detectLayer(World world, BlockPos from, BlockPos to, Consumer<Collection<BlockPos>> consumer) {
        ...
        if (hasFrame) {
            ...
            if (!frameCheck.test(mutable.set(from.getX(), height, to.getZ())))
                return MultiblockResult.error(mutable.toImmutable(), INVALID_WALL_FRAME);
        ...
        for (int x = from.getX() + 1; x < to.getX(); x++) {
            if (!wallCheck.test(mutable.set(x, height, from.getZ())))
                return MultiblockResult.error(mutable.toImmutable(), INVALID_WALL_BLOCK);
     ...
     *
     * @param nbt           Tag tag
     * @param controllerPos Position of the controller block entity, to store them relatively. If you wish to store absolutely, use {@link BlockPos#ORIGIN}.
     * @return Structure data, or null if invalid
     */
    @Nullable
    public T readFromTag(NbtCompound nbt, BlockPos controllerPos) {
     ...
     * @param offset  Offset position, for saving relatively. Use {@link BlockPos#ORIGIN} for absolute
     * @return Set of positions
     */
    protected static Collection<BlockPos> readPosList(NbtCompound rootTag, String key, BlockPos offset) {
