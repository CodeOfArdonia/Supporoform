...
public abstract class CastingBlockEntity extends TableBlockEntity implements SidedInventory, FluidUpdatePacket.IFluidPacketReceiver {
    // slots
    ...
    private static final Text NAME = TConstruct.makeTranslation("gui", "casting");
     ...
     */
    private boolean interactWithFluidItem(PlayerEntity player, Hand hand, ItemStack stack) {
        if (world == null) {
            return false;
        }
    ...
    /**
     * Called from {@link AbstractCastingBlock#onUse(BlockState, World, BlockPos, PlayerEntity, Hand, BlockHitResult)}
     *
    ...
    public void interact(PlayerEntity player, Hand hand) {
        ...
        if (world == null || world.isClient || (coolingTime >= 0 && timer > 0)) {
            return;
        }
        ...
        if (!input.isEmpty() && output.isEmpty()) {
            ...
            if (recipe != null) {
                ...
                if (recipe != null) {
                    setStack(INPUT, ItemStack.EMPTY);
                    ItemHandlerHelper.giveItemToPlayer(player, recipe.craft(moldingInventory), player.getInventory().selectedSlot);
                    return;
                }
            }
        ...
        if (input.isEmpty() && output.isEmpty()) {
            ...
            ItemHandlerHelper.giveItemToPlayer(player, stack, player.getInventory().selectedSlot);
    ...
    public void handleRedstone(boolean hasSignal) {
        if (lastRedstone != hasSignal) {
            if (hasSignal) {
                if (world != null) {
                    world.scheduleBlockTick(pos, this.getCachedState().getBlock(), 2);
                }
            }
    ...
    public void swap() {
        if (currentRecipe == null) {
            ...
            if (world != null) {
                world.playSound(null, getPos(), Sounds.CASTING_CLICKS.getSound(), SoundCategory.BLOCKS, 1.0f, 1.0f);
            }
        }
    }
     ...
     */
    private void serverTick(World level, BlockPos pos) {
        // no recipe
        ...
        if (coolingTime >= 0) {
            ...
            if (timer >= coolingTime) {
                ...
                if (currentRecipe.switchSlots() != lastRedstone) {
                    ...
                    setStack(INPUT, output);
                    level.playSound(null, getPos(), Sounds.CASTING_CLICKS.getSound(), SoundCategory.BLOCKS, 1.0f, 1.0f);
                } else {
                    if (currentRecipe.isConsumed()) {
                        setStack(INPUT, ItemStack.EMPTY);
                    }
                ...
                level.playSound(null, pos, Sounds.CASTING_COOLS.getSound(), SoundCategory.BLOCKS, 0.5f, 4f);
                reset();
            } else {
                updateAnalogSignal();
            }
     ...
     */
    private void clientTick(World level, BlockPos pos) {
        if (currentRecipe == null) {
            return;
        }
        // fully filled
     ...
     */
    public long initNewCasting(FluidStack fluid, IFluidHandler.FluidAction action) {
        if (this.currentRecipe != null || this.recipeName != null) {
            return 0;
        }

        boolean hasInput = !getStack(INPUT).isEmpty();
        boolean hasOutput = !getStack(OUTPUT).isEmpty();
        ...
        if (!hasOutput) {
            ...
            if (castingRecipe != null) {
                if (action == FluidAction.EXECUTE) {
                    ...
                    setStack(INPUT, getStack(OUTPUT));
     ...
     */
    private void loadRecipe(World level, Identifier name) {
    ...
    @Override
    public void setWorld(World pLevel) {
        super.setWorld(pLevel);
    ...
    public void saveSynced(NbtCompound tags) {
        ...
        tags.put(TAG_TANK, tank.writeToTag(new NbtCompound()));
    ...
    public void readNbt(NbtCompound tags) {
        ...
        if (tags.contains(TAG_RECIPE, NbtCompound.STRING_TYPE)) {
     ...
     */
    @Nullable
    public static <CAST extends CastingBlockEntity, RET extends BlockEntity> BlockEntityTicker<RET> getTicker(World level, BlockEntityType<RET> check, BlockEntityType<CAST> casting) {
        return BlockEntityHelper.castTicker(check, casting, level.isClient ? CLIENT_TICKER : SERVER_TICKER);
    }
    ...
    private static void playEmptySound(World world, BlockPos pos, PlayerEntity player, FluidStack transferred) {
        world.playSound(null, pos, FluidTransferHelper.getEmptySound(transferred), SoundCategory.BLOCKS, 1.0F, 1.0F);
        player.sendMessage(Text.translatable(KEY_FILLED, transferred.getAmount(), transferred.getDisplayName()), true);
    }

    private static void playFillSound(World world, BlockPos pos, PlayerEntity player, FluidStack transferred) {
        world.playSound(null, pos, FluidTransferHelper.getFillSound(transferred), SoundCategory.BLOCKS, 1.0F, 1.0F);
