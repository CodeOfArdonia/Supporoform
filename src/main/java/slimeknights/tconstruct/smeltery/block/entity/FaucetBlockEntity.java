...
public class FaucetBlockEntity extends MantleBlockEntity {
    ...
    @Override
    protected void saveSynced(NbtCompound compound) {
        super.saveSynced(compound);
        compound.putByte(TAG_STATE, (byte) this.faucetState.ordinal());
        if (!this.renderFluid.isEmpty()) {
            compound.put(TAG_RENDER_FLUID, this.renderFluid.writeToNBT(new NbtCompound()));
        }
    }
    ...
    public void writeNbt(NbtCompound compound) {
        ...
        if (!this.drained.isEmpty()) {
            compound.put(TAG_DRAINED, this.drained.writeToNBT(new NbtCompound()));
        }
    }
    ...
    public void readNbt(NbtCompound compound) {
        ...
        // fluids
        if (compound.contains(TAG_DRAINED, NbtElement.COMPOUND_TYPE)) {
            this.drained = FluidStack.loadFluidStackFromNBT(compound.getCompound(TAG_DRAINED));
        } else {
            this.drained = FluidStack.EMPTY;
        }
        if (compound.contains(TAG_RENDER_FLUID, NbtElement.COMPOUND_TYPE)) {
