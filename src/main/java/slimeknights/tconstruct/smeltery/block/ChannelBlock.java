...
public class ChannelBlock extends Block implements BlockEntityProvider {
    private static final Text SIDE_IN = TConstruct.makeTranslation("block", "channel.side.in");
    private static final Text SIDE_OUT = TConstruct.makeTranslation("block", "channel.side.out");
    private static final Text SIDE_NONE = TConstruct.makeTranslation("block", "channel.side.none");
    private static final Text DOWN_OUT = TConstruct.makeTranslation("block", "channel.down.out");
    private static final Text DOWN_NONE = TConstruct.makeTranslation("block", "channel.down.none");
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction facing, BlockState facingState, WorldAccess world, BlockPos currentPos, BlockPos facingPos) {
        ...
        if (facing == Direction.DOWN) {
            if (state.get(DOWN) && facingState.isAir()) {
    ...
    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World pLevel, BlockState pState, BlockEntityType<T> givenType) {
        return pLevel.isClient ? null : BlockEntityHelper.castTicker(givenType, TinkerSmeltery.channel.get(), ChannelBlockEntity.SERVER_TICKER);
    }
