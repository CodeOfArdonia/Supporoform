...
public final class TinkerSmeltery extends TinkerModule {
     ...
     */
    public static final ItemGroup TAB_SMELTERY = new SupplierCreativeTab(TConstruct.MOD_ID, "smeltery", () -> new ItemStack(TinkerSmeltery.smelteryController));
     ...
     */
    private static final Item.Settings SMELTERY_PROPS = new Item.Settings().tab(TAB_SMELTERY);
    private static final Function<Block, ? extends BlockItem> TOOLTIP_BLOCK_ITEM = (b) -> new BlockTooltipItem(b, SMELTERY_PROPS);

    /*
     * Blocks
     */
    public static final ItemObject<Block> grout = BLOCKS.register("grout", builder(Material.SAND, MaterialColor.COLOR_LIGHT_GRAY, BlockSoundGroup.SAND).strength(3.0f).friction(0.8F), TOOLTIP_BLOCK_ITEM);
    public static final ItemObject<Block> netherGrout = BLOCKS.register("nether_grout", builder(Material.SAND, BlockSoundGroup.SOUL_SOIL).strength(3.0f).friction(0.8F), TOOLTIP_BLOCK_ITEM);

    // seared blocks
    private static final Settings SEARED, TOUGH_SEARED, SEARED_GLASS, SEARED_SOUL_GLASS, SEARED_NON_SOLID, SEARED_LANTERN;

    static {
        // solid
        IntFunction<Settings> solidProps = factor ->
                builder(Material.STONE, MaterialColor.COLOR_GRAY, BlockSoundGroup.METAL).requiresCorrectToolForDrops().strength(3.0F * factor, 9.0F * factor)
        ...
        // non-solid
        Function<BlockSoundGroup, Settings> nonSolidProps = sound -> builder(Material.STONE, MaterialColor.COLOR_GRAY, sound)
        ...
        SEARED_GLASS = nonSolidProps.apply(BlockSoundGroup.GLASS);
        SEARED_SOUL_GLASS = nonSolidProps.apply(BlockSoundGroup.GLASS).noCollision().velocityMultiplier(0.1f).blockVision((state, getter, pos) -> true);
        SEARED_NON_SOLID = nonSolidProps.apply(BlockSoundGroup.METAL);
        SEARED_LANTERN = nonSolidProps.apply(BlockSoundGroup.LANTERN);
    }
    ...
    // scorched blocks
    private static final Settings SCORCHED, TOUGH_SCORCHED, SCORCHED_GLASS, SCORCHED_SOUL_GLASS, SCORCHED_NON_SOLID, SCORCHED_LANTERN;

    static {
        IntFunction<Settings> solidProps = factor -> builder(Material.STONE, MaterialColor.TERRACOTTA_BROWN, BlockSoundGroup.BASALT)
        ...
        Function<BlockSoundGroup, Settings> nonSolidProps = sound -> builder(Material.STONE, MaterialColor.TERRACOTTA_BROWN, sound)
                .requiresCorrectToolForDrops().strength(2.5F, 8.0F).noOcclusion()
                .isValidSpawn(Blocks::never).isRedstoneConductor(Blocks::never).isSuffocating(Blocks::never).isViewBlocking(Blocks::never);
        SCORCHED_GLASS = nonSolidProps.apply(BlockSoundGroup.GLASS);
        SCORCHED_SOUL_GLASS = nonSolidProps.apply(BlockSoundGroup.GLASS).noCollision().velocityMultiplier(0.1f).blockVision((state, getter, pos) -> true);
        SCORCHED_NON_SOLID = nonSolidProps.apply(BlockSoundGroup.BASALT);
        SCORCHED_LANTERN = nonSolidProps.apply(BlockSoundGroup.LANTERN);
    }
    ...
    // seared
    public static final EnumObject<TankType, SearedTankBlock> searedTank = BLOCKS.registerEnum("seared", TankType.values(), type -> new SearedTankBlock(SEARED_NON_SOLID, type.getCapacity(), PistonBehavior.DESTROY), b -> new TankItem(b, SMELTERY_PROPS, true));
    ...
    // scorched
    public static final EnumObject<TankType, SearedTankBlock> scorchedTank = BLOCKS.registerEnum("scorched", TankType.values(), type -> new SearedTankBlock(SCORCHED_NON_SOLID, type.getCapacity(), PistonBehavior.DESTROY), b -> new TankItem(b, SMELTERY_PROPS, true));
    ...
    static {
        Supplier<Settings> seared = () -> builder(Material.STONE, MaterialColor.COLOR_GRAY, BlockSoundGroup.METAL).requiresCorrectToolForDrops().strength(8.0F, 28F).lightLevel(s -> s.getValue(ControllerBlock.ACTIVE) ? 13 : 0);
        Supplier<Settings> scorched = () -> builder(Material.STONE, MaterialColor.TERRACOTTA_BROWN, BlockSoundGroup.BASALT).requiresCorrectToolForDrops().strength(9.0F, 35f).lightLevel(s -> s.getValue(ControllerBlock.ACTIVE) ? 13 : 0);
        ...
        // tiny
        searedMelter = BLOCKS.register("seared_melter", () -> new MelterBlock(seared.get().nonOpaque()), TOOLTIP_BLOCK_ITEM);
        searedHeater = BLOCKS.register("seared_heater", () -> new HeaterBlock(seared.get()), TOOLTIP_BLOCK_ITEM);
        scorchedAlloyer = BLOCKS.register("scorched_alloyer", () -> new AlloyerBlock(scorched.get().nonOpaque()), TOOLTIP_BLOCK_ITEM);
    }
    ...
    public static final ItemObject<Item> copperCan = ITEMS.register("copper_can", () -> new CopperCanItem(new Item.Settings().maxCount(16).tab(TAB_SMELTERY)));
     ...
     * Inventory
     */
    public static final RegistryObject<ScreenHandlerType<MelterContainerMenu>> melterContainer = MENUS.register("melter", MelterContainerMenu::new);
    public static final RegistryObject<ScreenHandlerType<HeatingStructureContainerMenu>> smelteryContainer = MENUS.register("smeltery", HeatingStructureContainerMenu::new);
    public static final RegistryObject<ScreenHandlerType<SingleItemContainerMenu>> singleItemContainer = MENUS.register("single_item", SingleItemContainerMenu::new);
    public static final RegistryObject<ScreenHandlerType<AlloyerContainerMenu>> alloyerContainer = MENUS.register("alloyer", AlloyerContainerMenu::new);
