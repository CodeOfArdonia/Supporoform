...
public class TinkerStationContainerWrapper implements IMutableTinkerStationContainer {
    ...
    private MaterialRecipe findMaterialRecipe(ItemStack stack) {
        // must have world
        World world = this.station.getWorld();
        if (world == null) {
            return null;
        }
        // try last recipe
        ...
        Optional<MaterialRecipe> newRecipe = world.getRecipeManager().getFirstMatch(TinkerRecipeTypes.MATERIAL.get(), inv, world);
    ...
    public ItemStack getInput(int index) {
        ...
        return this.station.getStack(index + TinkerStationBlockEntity.INPUT_SLOT);
    }
    ...
    public void setInput(int index, ItemStack stack) {
        if (index >= 0 && index < this.station.getInputCount()) {
            this.station.setStack(index + TinkerStationBlockEntity.INPUT_SLOT, stack);
        }
    }
