...
public class DynamicContainerScreen extends ModuleScreen {
    ...
    protected final ScreenHandler container;

    public DynamicContainerScreen(MultiModuleScreen<?> parent, ScreenHandler container, PlayerInventory playerInventory, Text title) {
    ...
    protected void drawBackground(DrawContext context, float partialTicks, int mouseX, int mouseY) {
        ...
        for (y = 0; y < fullRows * slot.h && y < this.backgroundHeight; y += slot.h) {
            slot.drawScaledX(context, this.x, this.y + y, w);
        }

        // draw partial row and unused slots
        int slotsLeft = (this.lastSlotId - this.firstSlotId) % this.columns;
        if (slotsLeft > 0) {
            slot.drawScaledX(context, this.x, this.y + y, slotsLeft * slot.w);
            // empty slots that don't exist
            slotEmpty.drawScaledX(context, this.x + slotsLeft * slot.w, this.y + y, w - slotsLeft * slot.w);
        }
    }
}
