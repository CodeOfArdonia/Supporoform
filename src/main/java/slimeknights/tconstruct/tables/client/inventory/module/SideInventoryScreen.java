...
public class SideInventoryScreen<P extends MultiModuleScreen<?>, C extends ScreenHandler> extends ModuleScreen<P, C> {
    ...
    protected boolean shouldDrawName() {
        return this.handler instanceof BaseContainerMenu<?>;
    }
    ...
    protected void drawForeground(DrawContext context, int mouseX, int mouseY) {
        if (this.shouldDrawName()) {
            context.drawText(this.textRenderer, this.getTitle().getString(), this.border.w, this.border.h - 1, 0x404040, false);
        }
    }
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        if (this.shouldDrawName()) {
            this.textBackground.drawScaledX(context, x, y, midW);
            y += this.textBackground.h;
        }

        //this.minecraft.getTextureManager().bind(GENERIC_INVENTORY); TODO: needed?
        this.drawSlots(context, x, y);
    ...
    protected int drawSlots(DrawContext context, int xPos, int yPos) {
        ...
        for (y = 0; y < fullRows * this.slot.h && y < height; y += this.slot.h) {
            this.slot.drawScaledX(context, xPos, yPos + y, width);
        }
        ...
        if (slotsLeft > 0) {
            this.slot.drawScaledX(context, xPos, yPos + y, slotsLeft * this.slot.w);
            // empty slots that don't exist
            this.slotEmpty.drawScaledX(context, xPos + slotsLeft * this.slot.w, yPos + y, width - slotsLeft * this.slot.w);
        }
