...
import slimeknights.mantle.client.screen.*;
...
public class InfoPanelScreen extends ModuleScreen {
     ...
     */
    private static final Text DEFAULT_CAPTION = TConstruct.makeTranslation("gui", "caption").formatted(Formatting.UNDERLINE);
    ...
    public void setCaption(Text caption) {
        this.caption = caption.copy().formatted(Formatting.UNDERLINE);
        this.updateSliderParameters();
    }
    ...
    public void drawMouseoverTooltip(DrawContext context, int mouseX, int mouseY) {
        ...
        if (this.hasTooltips() && mouseX >= this.guiRight() - this.border.w - this.textRenderer.getWidth("?") / 2 && mouseX < this.guiRight()
            ...
            context.drawTooltip(this.textRenderer, this.textRenderer.wrapLines(Text.translatable("gui.tconstruct.general.hover"), 150).stream().map(x -> (Text) x).toList(), mouseX - 155, mouseY);
        }
        ...
        List<Text> lines = this.textRenderer.wrapLines(this.tooltips.get(i), w).stream().map(x -> (Text) x).toList();

        context.drawTooltip(this.textRenderer, lines, mouseX, (mouseY - lines.size() * this.getScaledFontHeight() / 2));
    }
    ...
    protected void drawBackground(DrawContext context, float partialTicks, int mouseX, int mouseY) {
        ...
        if (this.hasTooltips()) {
            context.drawTextWithShadow(this.textRenderer, "?", guiRight() - this.border.w - this.textRenderer.getWidth("?") / 2, this.y + 5, 0xff5f5f5f);
        }
