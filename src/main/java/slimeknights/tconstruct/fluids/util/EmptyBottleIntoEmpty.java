...
public record EmptyBottleIntoEmpty(Supplier<Item> empty, CauldronBehavior fallback) implements CauldronBehavior {
    @Override
    public ActionResult interact(BlockState state, World level, BlockPos pos, PlayerEntity player, Hand hand, ItemStack stack) {
        ...
        if (!level.isClient) {
            ...
            level.setBlockState(pos, Blocks.WATER_CAULDRON.getDefaultState());
