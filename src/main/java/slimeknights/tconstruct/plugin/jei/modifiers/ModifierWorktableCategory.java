...
public class ModifierWorktableCategory implements IRecipeCategory<IModifierWorktableRecipe> {
    ...
    private static final Text TITLE = TConstruct.makeTranslation("jei", "modifier_worktable.title");
    ...
    @Override
    public void draw(IModifierWorktableRecipe recipe, IRecipeSlotsView slots, MatrixStack matrixStack, double mouseX, double mouseY) {
        if (recipe.getInputTools().isEmpty()) {
            this.toolIcon.draw(matrixStack, 23, 16);
        }
        for (int i = 0; i < 2; i++) {
            List<ItemStack> stacks = recipe.getDisplayItems(i);
            if (stacks.isEmpty()) {
                this.slotIcons[i].draw(matrixStack, 43 + i * 18, 16);
            }
        }
        MinecraftClient.getInstance().textRenderer.draw(matrixStack, recipe.getTitle(), 3, 2, 0x404040);
    }

    @Override
    public List<Text> getTooltipStrings(IModifierWorktableRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
