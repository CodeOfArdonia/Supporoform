...
public class EntityMeltingRecipeCategory implements IRecipeCategory<EntityMeltingRecipe> {
    ...
    private static final Text TITLE = TConstruct.makeTranslation("jei", "entity_melting.title");
    private static final String KEY_PER_HEARTS = TConstruct.makeTranslationKey("jei", "entity_melting.per_hearts");
    private static final Text TOOLTIP_PER_HEART = Text.translatable(TConstruct.makeTranslationKey("jei", "entity_melting.per_heart")).formatted(Formatting.GRAY);
    ...
    @Override
    public void draw(EntityMeltingRecipe recipe, IRecipeSlotsView slot, MatrixStack matrices, double mouseX, double mouseY) {
        ...
        int x = 84 - fontRenderer.getWidth(damage);
    ...
    private record FluidTooltip(int damage) implements IRecipeTooltipReplacement {
        @Override
        public void addMiddleLines(IRecipeSlotView recipeSlotView, List<Text> list) {
            // add fluid units
            ...
            if (damage == 2) {
                ...
                list.add(Text.translatable(KEY_PER_HEARTS, damage / 2f).formatted(Formatting.GRAY));
            }
        }
    }
}
