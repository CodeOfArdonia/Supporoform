...
public abstract class AbstractMeltingCategory implements IRecipeCategory<MeltingRecipe> {
    ...
    @Override
    public List<Text> getTooltipStrings(MeltingRecipe recipe, IRecipeSlotsView slots, double mouseXD, double mouseYD) {
        ...
        if (GuiUtil.isHovered(mouseX, mouseY, 56, 18, 24, 17)) {
            return Collections.singletonList(Text.translatable(KEY_COOLING_TIME, recipe.getTime() / 4));
        }
