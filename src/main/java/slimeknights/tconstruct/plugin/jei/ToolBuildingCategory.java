...
public class ToolBuildingCategory implements IRecipeCategory<ToolBuildingRecipe> {
    ...
    private static final Text TITLE = TConstruct.makeTranslation("jei", "tinkering.tool_building");
    @Getter
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, ToolBuildingRecipe recipe, IFocusGroup focuses) {
        List<List<ItemStack>> partsAndExtras = Stream.concat(recipe.getAllToolParts().stream(),
                recipe.getExtraRequirements().stream().map(ingredient -> Arrays.asList(ingredient.getMatchingStacks()))).toList();
    ...
    @Override
    public void draw(ToolBuildingRecipe recipe, IRecipeSlotsView recipeSlotsView, MatrixStack stack, double mouseX, double mouseY) {
        // first, draw the item background
        ItemStack outputStack = recipe.getOutput() instanceof IModifiableDisplay modifiable ? modifiable.getRenderTool() : recipe.getOutput().asItem().getDefaultInstance();
        MatrixStack renderPose = RenderSystem.getModelViewStack();
        renderPose.push();
        renderPose.multiplyPositionMatrix(stack.peek().getPositionMatrix());
        renderPose.translate(5, 6.5, 0);
        renderPose.scale(3.7f, 3.7f, 1.0f);
        MinecraftClient.getInstance().getItemRenderer().renderGuiItem(outputStack, 0, 0);
        renderPose.pop();
    ...
    @Override
    public List<Text> getTooltipStrings(ToolBuildingRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
