...
public class RayTracer {
    ...
    public static BlockHitResult retrace(PlayerEntity player, RaycastContext.ShapeType blockMode, RaycastContext.FluidHandling fluidMode) {
        return player.getWorld().raycast(new RaycastContext(getStartVector(player), getEndVector(player), blockMode, fluidMode, player));
    }
    ...
    public static double getBlockReachDistance(PlayerEntity player) {
        return player.getWorld().isClient ? ClientOnly.getBlockReachDistanceClient() : player instanceof ServerPlayerEntity ? getBlockReachDistanceServer((ServerPlayerEntity) player) : 5D;
    }
    ...
    private static double getBlockReachDistanceServer(ServerPlayerEntity player) {
        return player.getAttributeValue(PortingLibAttributes.BLOCK_REACH);
    }
