...
public abstract class AbstractProtectionModifier<T extends ModifierMaxLevel> extends Modifier implements EquipmentChangeModifierHook {
    ...
    public void onUnequip(IToolStackView tool, ModifierEntry modifier, EquipmentChangeContext context) {
        ...
        if ((this.allowClient || !context.getEntity().getWorld().isClient) && ModifierUtil.validArmorSlot(tool, slot)) {
    ...
    public void onEquip(IToolStackView tool, ModifierEntry modifier, EquipmentChangeContext context) {
        EquipmentSlot slot = context.getChangedSlot();
        if ((this.allowClient || !context.getEntity().getWorld().isClient) && ModifierUtil.validArmorSlot(tool, slot) && !tool.isBroken()) {
            float scaledLevel = modifier.getEffectiveLevel();
            context.getTinkerData().ifPresent(data -> {
                // add ourself to the data
                this.set(data.computeIfAbsent(this.key), slot, scaledLevel, context);
            });
        }
    }
}
