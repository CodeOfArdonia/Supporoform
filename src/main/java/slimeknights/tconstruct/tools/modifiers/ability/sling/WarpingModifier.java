...
public class WarpingModifier extends SlingModifier {
    ...
    public void onStoppedUsing(IToolStackView tool, ModifierEntry modifier, LivingEntity entity, int timeLeft) {
        ...
        if (!entity.getWorld().isClient && entity instanceof ServerPlayerEntity player) {
            float f = this.getForce(tool, modifier, entity, timeLeft, false) * 6;
            if (f > 0) {
                ...
                while (Math.abs(offX) > .5 || Math.abs(offY) > .5 || Math.abs(offZ) > .5) { // while not too close to player
                    BlockPos posAttempt = BlockPos.ofFloored(player.getX() + offX, player.getY() + offY, player.getZ() + offZ);
                    // if we do not have a position yet, see if this one is valid
                    if (furthestPos == null) {
                        if (player.getWorld().getWorldBorder().contains(posAttempt) && !player.getWorld().getBlockState(posAttempt).shouldSuffocate(player.getWorld(), posAttempt)) {
                            furthestPos = posAttempt;
                        }
                    } else {
                        // if we already have a position, clear if the new one is unbreakable
                        if (player.getWorld().getBlockState(posAttempt).getHardness(player.getWorld(), posAttempt) == -1) {
                            furthestPos = null;
                        }
                    }
                ...
                if (furthestPos != null) {
                    ...
                    if (!event.isCanceled()) {
                        player.requestTeleport(event.getTargetX(), event.getTargetY(), event.getTargetZ());

                        // particle effect from EnderPearlEntity
                        for (int i = 0; i < 32; ++i) {
                            entity.getWorld().addParticle(ParticleTypes.PORTAL, player.getX(), player.getY() + entity.getWorld().random.nextDouble() * 2.0D, player.getZ(), entity.level.random.nextGaussian(), 0.0D, entity.level.random.nextGaussian());
                        }
                        entity.getWorld().playSound(null, player.getX(), player.getY(), player.getZ(), Sounds.SLIME_SLING_TELEPORT.getSound(), player.getSoundCategory(), 1f, 1f);
            ...
            entity.getWorld().playSound(null, entity.getX(), entity.getY(), entity.getZ(), Sounds.SLIME_SLING_TELEPORT.getSound(), entity.getSoundCategory(), 1, 0.5f);
        }
    }
}
