...
public class MeltingModifier extends NoLevelsModifier implements MeleeHitModifierHook, ProcessLootModifierHook {
    ...
    private static FluidStack meltItem(ItemStack stack, World world) {
        ...
        if (recipe == null || !recipe.matches(inventory, world)) {
            recipe = world.getRecipeManager().getFirstMatch(TinkerRecipeTypes.MELTING.get(), inventory, world).orElse(null);
            if (recipe == null) {
    ...
    public void processLoot(IToolStackView tool, ModifierEntry modifier, List<ItemStack> generatedLoot, LootContext context) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!output.isEmpty() && (current.isEmpty() || current.isFluidEqual(output))) {
                ...
                if (maxCopies > 0) {
                    // fill fluid
                    long amount = output.getAmount() * maxCopies;
                    if (current.isEmpty()) {
    ...
    public void afterMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damageDealt) {
        ...
        if (damageDealt > 0 && context.isFullyCharged()) {
            ...
            if (target != null) {
                EntityMeltingRecipe recipe = EntityMeltingRecipeCache.findRecipe(context.getAttacker().getWorld().getRecipeManager(), target.getType());
                FluidStack output;
                int damagePerOutput;
                if (recipe != null) {
