...
public class SpringyModifier extends Modifier implements MeleeHitModifierHook, OnAttackedModifierHook {
    ...
    public void onAttacked(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float amount, boolean isDirectDamage) {
        ...
        if (isDirectDamage && !user.getWorld().isClient && attacker instanceof LivingEntity livingAttacker && SlotInChargeModule.isInCharge(context.getTinkerData(), SLOT_IN_CHARGE, slotType)) {
            ...
            if (bestBonus > 0) {
                ...
                livingAttacker.takeKnockback(bestBonus, -MathHelper.sin(angle), MathHelper.cos(angle));
            }
        }
    }
