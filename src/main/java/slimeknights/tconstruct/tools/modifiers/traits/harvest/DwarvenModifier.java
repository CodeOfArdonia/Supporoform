...
public class DwarvenModifier extends Modifier implements ConditionalStatModifierHook, BreakSpeedModifierHook, TooltipModifierHook {
    private static final Text MINING_SPEED = TConstruct.makeTranslation("modifier", "dwarven.mining_speed");
    private static final Text VELOCITY = TConstruct.makeTranslation("modifier", "dwarven.velocity");
    /**
     ...
     */
    private static float getBoost(World world, float y, ModifierEntry entry, float baseSpeed, float bonus) {
        ...
        if (y < BOOST_DISTANCE) {
            float scale = MathHelper.clamp((BOOST_DISTANCE - y) / BOOST_DISTANCE, 0, 2);
        ...
        float baselineDebuff = Math.max(world.getTopY() - (DEBUFF_RANGE + BOOST_DISTANCE), 96);
        if (y > baselineDebuff) {
    ...
    public float modifyStat(IToolStackView tool, ModifierEntry modifier, LivingEntity living, FloatToolStat stat, float baseValue, float multiplier) {
        if (stat == ToolStats.VELOCITY) {
            return getBoost(living.world, (float) living.getY(), modifier, baseValue, multiplier * VELOCITY_BONUS);
        }
        return baseValue;
    }
    ...
    public void addTooltip(IToolStackView tool, ModifierEntry modifier, @Nullable PlayerEntity player, List<Text> tooltip, TooltipKey key, TooltipContext tooltipFlag) {
        ...
        if (type != null) {
            Text prefix = type == ToolType.RANGED ? VELOCITY : MINING_SPEED;
            ...
            if (player != null && key == TooltipKey.SHIFT) {
                ...
                boost = getBoost(player.world, (float) player.getY(), modifier, 1, baseBoost) - 1;
                if (boost < 0) {
