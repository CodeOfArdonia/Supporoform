...
public class HolyModifier extends Modifier implements ProjectileHitModifierHook {
    ...
    public boolean onProjectileHitEntity(ModifierNBT modifiers, NamespacedNBT persistentData, ModifierEntry modifier, ProjectileEntity projectile, EntityHitResult hit, @Nullable LivingEntity attacker, @Nullable LivingEntity target) {
        if (target != null && target.getGroup() == EntityGroup.UNDEAD && projectile instanceof PersistentProjectileEntity arrow) {
            arrow.setDamage(arrow.getDamage() + modifier.getLevel() / 2f);
        }
        return false;
    }
}
