...
public class TemperateModifier extends Modifier implements ConditionalStatModifierHook, ToolDamageModifierHook, BreakSpeedModifierHook, TooltipModifierHook, ProtectionModifierHook {
    ...
    private static final Text SPEED = TConstruct.makeTranslation("modifier", "temperate.speed");
    private static final Text REINFORCED = TConstruct.makeTranslation("modifier", "temperate.reinforced");
    ...
    private static float getBonus(LivingEntity living, BlockPos pos) {
        ...
        return (living.world.getBiome(pos).value().getTemperature(pos) - BASELINE_TEMPERATURE);
    }
    ...
    public int onDamageTool(IToolStackView tool, ModifierEntry modifier, int amount, @Nullable LivingEntity holder) {
        ...
        if (holder != null) {
            float bonus = getBonus(holder, holder.getBlockPos());
            if (bonus > 0) {
    ...
    public float modifyStat(IToolStackView tool, ModifierEntry modifier, LivingEntity living, FloatToolStat stat, float baseValue, float multiplier) {
        ...
        if (stat == ToolStats.DRAW_SPEED) {
            float bonus = getBonus(living, living.getBlockPos());
            if (bonus < 0) {
    ...
    public float getProtectionModifier(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float modifierValue) {
        if (DamageSourcePredicate.CAN_PROTECT.matches(source)) {
            ...
            float bonus = getBonus(target, target.getBlockPos());
            if (bonus < 0) {
    ...
    @Override
    public void addTooltip(IToolStackView tool, ModifierEntry entry, @Nullable PlayerEntity player, List<Text> tooltip, TooltipKey key, TooltipContext tooltipFlag) {
        ToolType type = ToolType.from(tool.getItem(), ToolType.NO_MELEE);
        if (type != null) {
            float bonus = entry.getEffectiveLevel();
            if (player != null && key == TooltipKey.SHIFT) {
                bonus *= getBonus(player, player.getBlockPos());
            } else {
                bonus *= -1;
            }
            if (bonus < -0.01f) {
            ...
            if (bonus > 0.01f) {
                tooltip.add(this.applyStyle(Text.literal(Util.PERCENT_FORMAT.format(diminishingPercent(bonus * 2 / MAX_TEMPERATURE)) + " ").append(REINFORCED)));
            }
        }
    }
}
