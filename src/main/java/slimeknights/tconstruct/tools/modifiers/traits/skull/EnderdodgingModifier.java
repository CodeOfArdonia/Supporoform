...
public class EnderdodgingModifier extends NoLevelsModifier implements DamageBlockModifierHook, OnAttackedModifierHook {
    ...
    public boolean isDamageBlocked(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float amount) {
        ...
        if (!self.hasStatusEffect(TinkerModifiers.teleportCooldownEffect.get()) && source instanceof IndirectEntityDamageSource) {
    ...
    public void onAttacked(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float amount, boolean isDirectDamage) {
        ...
        LivingEntity self = context.getEntity();
        if (!self.hasStatusEffect(TinkerModifiers.teleportCooldownEffect.get()) && source.getAttacker() instanceof LivingEntity && RANDOM.nextInt(10) == 0) {
            if (TeleportHelper.randomNearbyTeleport(context.getEntity(), FACTORY)) {
