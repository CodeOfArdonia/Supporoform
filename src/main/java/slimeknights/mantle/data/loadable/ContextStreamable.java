...
public interface ContextStreamable<T> extends Streamable<T> {
     ...
     */
    T decode(PacketByteBuf buffer, TypedMap context);

    /**
     * Contextless implementation of {@link #decode(PacketByteBuf, TypedMap)} for {@link Streamable}.
     */
    @Override
    default T decode(PacketByteBuf buffer) {
        return this.decode(buffer, TypedMap.empty());
    }
}
