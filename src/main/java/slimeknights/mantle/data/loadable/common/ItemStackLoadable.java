...
public class ItemStackLoadable {
     ...
     * Field for item stack count that allows empty
     */
    private static final LoadableField<NbtCompound, ItemStack> NBT = NBTLoadable.ALLOW_STRING.nullableField("nbt", ItemStack::getNbt);
     ...
     */
    private static ItemStack makeStack(Item item, int count, @Nullable NbtCompound nbt) {
    ...
    private enum NBTStack implements RecordLoadable<ItemStack> {
        ...
        public JsonElement serialize(ItemStack stack) {
            if ((this == FIXED_COUNT || stack.getCount() == 1) && !stack.hasNbt()) {
        ...
        @Override
        public ItemStack decode(PacketByteBuf buffer, TypedMap context) {
            ...
            ItemStack stack = new ItemStack(item, count);
            stack.getOrCreateNbt().put("share_tag", nbt);
            return stack;
        }

        @Override
        public void encode(PacketByteBuf buffer, ItemStack stack) throws EncoderException {
            ITEM.encode(buffer, stack);
            if (this == READ_COUNT) {
                COUNT.encode(buffer, stack);
            }
            buffer.writeNbt(stack.getOrCreateNbt().getCompound("share_tag"));
        }
    }
}
