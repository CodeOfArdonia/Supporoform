...
public class EitherLoadable {
    ...
    public static class TypedBuilder<T extends IAmLoadable> {
        ...
        public TypedBuilder<T> key(String key, RecordLoadable<? extends T> loadable) {
            this.keys.add(new KeyOption<>(key, loadable));
            return this;
        }
        ...
        public Loadable<T> build(Streamable<T> network) {
            return new Typing<>(List.of(network), this.getKeys(), this.array, this.primitive);
        }
        ...
        public Loadable<T> build() {
            ...
            if (this.array != null) {
                builder.add((Loadable<T>) this.array);
            }
            if (this.primitive != null) {
                builder.add((Loadable<T>) this.primitive);
            }
            return new Typing<>(builder.build(), this.getKeys(), this.array, this.primitive);
        }
    }
    ...
    public static class RecordBuilder<T extends IAmLoadable.Record> {
        ...
        public RecordLoadable<T> build(ContextStreamable<T> network) {
            return new Record<>(List.of(network), this.getKeys());
        }
    ...
    private interface EitherImpl<T extends IAmLoadable, L extends Streamable<T>> extends Loadable<T> {
         ...
         */
        default L loadableFromNetwork(PacketByteBuf buffer) {
            List<L> networks = this.network();
        ...
        @Override
        default void encode(PacketByteBuf buffer, T object) {
            List<L> networks = this.network();
            // size 1 means we have a fixed network logic, use that
            if (networks.size() == 1) {
     ...
     */
    private record Typing<T extends IAmLoadable>(List<Streamable<T>> network, List<KeyOption<T>> keys,
                                                 @Nullable Loadable<? extends T> array,
                                                 @Nullable Loadable<? extends T> primitive) implements EitherImpl<T, Streamable<T>> {
        @Override
        public T convert(JsonElement element, String key) {
            if (this.array != null && element.isJsonArray()) {
                return this.array.convert(element, key);
            }
            if (this.primitive != null && element.isJsonPrimitive()) {
                return this.primitive.convert(element, key);
            }
            if (!this.keys.isEmpty()) {
                return this.deserializeObject(element, TypedMap.empty(), key);
            }
        ...
        public T decode(PacketByteBuf buffer) {
            return this.loadableFromNetwork(buffer).decode(buffer);
        }
    }
     ...
     */
    private record Record<T extends IAmLoadable.Record>(List<ContextStreamable<T>> network,
                                                        List<KeyOption<T>> keys) implements RecordLoadable<T>, EitherImpl<T, ContextStreamable<T>> {
        @Override
        public T deserialize(JsonObject json, TypedMap context) {
            return this.deserializeObject(json, context, "[root]");
        }
        ...
        public T decode(PacketByteBuf buffer, TypedMap context) {
            return this.loadableFromNetwork(buffer).decode(buffer, context);
        }
