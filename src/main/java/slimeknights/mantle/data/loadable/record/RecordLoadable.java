...
public interface RecordLoadable<T> extends Loadable<T>, IGenericLoader<T>, ContextStreamable<T> {
    ...
    default T deserialize(JsonObject json) {
        return this.deserialize(json, TypedMap.empty());
    }

    @Override
    default T convert(JsonElement element, String key) {
        return this.deserialize(JsonHelper.asObject(element, key));
    }
    ...
    @Override
    default T fromNetwork(PacketByteBuf buffer) {
        return this.decode(buffer);
    }

    /**
     * @deprecated use {@link #encode(PacketByteBuf, Object)}
     */
    @Deprecated(forRemoval = true)
    @Override
    default void toNetwork(T object, PacketByteBuf buffer) {
        this.encode(buffer, object);
    }
