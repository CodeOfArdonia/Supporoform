...
public interface ResourceLocationLoadable<T> extends StringLoadable<T> {
     ...
     */
    T fromKey(Identifier name, String key);

    @Override
    default T parseString(String value, String key) {
        return this.fromKey(Loadables.RESOURCE_LOCATION.parseString(value, key), key);
    }

    @Override
    default T convert(JsonElement element, String key) {
        return this.fromKey(Loadables.RESOURCE_LOCATION.convert(element, key), key);
    }
