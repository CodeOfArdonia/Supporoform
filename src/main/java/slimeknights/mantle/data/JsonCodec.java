...
public interface JsonCodec<T> extends Codec<T> {
    ...
    default <O> DataResult<Pair<T, O>> decode(DynamicOps<O> ops, O input) {
        try {
            return DataResult.success(Pair.of(this.deserialize(ops.convertTo(JsonOps.INSTANCE, input)), input));
        } catch (JsonParseException e) {
            Mantle.logger.warn("Unable to decode {}", this.codecError(), e);
    ...
    default <O> DataResult<O> encode(T input, DynamicOps<O> ops, O prefix) {
        try {
            return DataResult.success(JsonOps.INSTANCE.convertTo(ops, this.serialize(input)));
        } catch (JsonParseException e) {
            Mantle.logger.warn("Unable to encode {}", this.codecError(), e);
            return DataResult.error(e::getMessage);
        }
    }
    ...
    record GsonCodec<T>(String name, Gson gson, Class<T> classType) implements JsonCodec<T> {
        ...
        public T deserialize(JsonElement element) {
            return this.gson.fromJson(element, this.classType);
        }
