            ...
            try {
                ...
                if (block != Blocks.AIR) {
                    ...
                    for (Entry<String, JsonElement> variant : variants.entrySet()) {
                        // parse fluid
                        FaucetFluid fluid = FaucetFluid.fromJson(JsonHelper.asObject(variant.getValue(), variant.getKey()), this.defaultFluid);
                        validStates.stream().filter(ModelLoader.stateKeyToPredicate(container, variant.getKey())).forEach(state -> this.fluidMap.put(state, fluid));
                    }
                } else {
