...
public class ContentListing extends PageContent {
    ...
    public int getEntriesInColumn(SectionData sectionData) {
        ...
        if (this.subText != null) {
            yOff += sectionData.parent.fontRenderer.getWrappedLinesHeight(this.subText, BookScreen.PAGE_WIDTH) * 12 / 9;
        }
        ...
        for (List<TextData> column : this.entries) {
            ...
            for (TextData data : column) {
                ...
                if (text.isEmpty()) {
                    ...
                    int height;
                    if (data.bold) {
                        height = TextDataRenderer.getLinesForString(text, Formatting.BOLD.toString(), width, "", this.parent.parent.parent.fontRenderer) * LINE_HEIGHT;
                    } else {
