...
public class SelectionElement extends SizedBookElement {
    ...
    public void draw(DrawContext graphics, int mouseX, int mouseY, float partialTicks, TextRenderer fontRenderer) {
        ...
        if (this.section.parent.appearance.drawSectionListText) {
            ...
            for (int i = 0; i < splitTitle.length; i++) {
                ...
                graphics.drawText(fontRenderer, splitTitle[i], textX, textY, hover ? 0xFF000000 : 0x7F000000, false);
            }
        }
    }
    ...
    public void drawOverlay(DrawContext graphics, int mouseX, int mouseY, float partialTicks, TextRenderer fontRenderer) {
        if (this.section != null && this.isHovered(mouseX, mouseY)) {
            ...
            text.add(Text.literal(this.section.getTitle()));

            if (!this.section.isUnlocked(this.parent.advancementCache)) {
                text.add(Text.literal("Locked").formatted(Formatting.RED));
                text.add(Text.literal("Requirements:"));

                for (String requirement : this.section.requirements) {
                    text.add(Text.literal(requirement));
                }
            }
