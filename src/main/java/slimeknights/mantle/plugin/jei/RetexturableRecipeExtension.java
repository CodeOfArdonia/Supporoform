...
public class RetexturableRecipeExtension implements ICraftingCategoryExtension {
    ...
    RetexturableRecipeExtension(ShapedRetexturedRecipe recipe) {
        ...
        DynamicRegistryManager access = Objects.requireNonNull(SafeClientAccess.getRegistryAccess());
        List<ItemStack> displayOutputs = Arrays.stream(texture.getMatchingStacks())
        ...
        this.displayOutputs = displayOutputs.isEmpty() ? List.of(this.recipe.getOutput(access)) : displayOutputs;
    ...
    private static boolean ingredientsMatch(Ingredient left, Ingredient right) {
        ItemStack[] leftStacks = left.getMatchingStacks();
        ItemStack[] rightStacks = right.getMatchingStacks();
        if (leftStacks.length != rightStacks.length) {
            return false;
        }
        for (int i = 0; i < leftStacks.length; i++) {
            if (!ItemStack.canCombine(leftStacks[i], rightStacks[i])) {
                return false;
            }
        }
        return true;
...
//    guiItemStacks.addTooltipCallback(this);
        ...
        builder.addInvisibleIngredients(RecipeIngredientRole.OUTPUT).addItemStack(this.recipe.getOutput(Objects.requireNonNull(SafeClientAccess.getRegistryAccess())));

        // add the itemstacks to the grid
        List<List<ItemStack>> inputStacks = this.recipe.getIngredients().stream().map(ingredient -> List.of(ingredient.getMatchingStacks())).toList();
        int width = this.recipe.getWidth();
        int height = this.recipe.getHeight();
        List<IRecipeSlotBuilder> inputs = craftingGridHelper.createAndSetInputs(builder, VanillaTypes.ITEM_STACK, inputStacks, this.recipe.getWidth(), this.recipe.getHeight());
        ...
        if (inputs.size() != 9) {
            ...
            builder.createFocusLink(Streams.concat(Stream.of(output), Arrays.stream(this.textureSlots).mapToObj(i -> inputs.get(getCraftingIndex(i, width, height)))).toArray(IRecipeSlotBuilder[]::new));
        }
    }
