...
public abstract class SingleItemHandler<T extends MantleBlockEntity> implements IItemHandlerModifiable {
        ...
        if (slot == 0) {
            ...
            if (current.isEmpty()) {
                if (this.isItemValid(slot, stack)) {
                    ...
                    int size = Math.min(stack.getCount(), this.getSlotLimit(0));
                    if (!simulate) {
                ...
                int added = Math.min(stack.getCount(), this.getSlotLimit(0) - current.getCount());
                if (added > 0) {
                    if (!simulate) {
                        current.increment(added);
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (amount < this.stack.getCount()) {
            ItemStack result = ItemHandlerHelper.copyStackWithSize(this.stack, amount);
            if (!simulate) {
                this.setStack(ItemHandlerHelper.copyStackWithSize(this.stack, this.stack.getCount() - amount));
            }
            return result;
        }
