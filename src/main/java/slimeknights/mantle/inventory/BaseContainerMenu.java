...
public class BaseContainerMenu<TILE extends BlockEntity> extends ScreenHandler {
    ...
    protected BaseContainerMenu(ScreenHandlerType<?> type, int id, @Nullable PlayerInventory inv, @Nullable TILE tile) {
        super(type, id);
        this.inv = inv;
        this.tile = tile;
    }
    ...
    public void syncOnOpen(ServerPlayerEntity playerOpened) {
        ...
        for (PlayerEntity player : server.getPlayers()) {
            ...
            if (player.currentScreenHandler instanceof BaseContainerMenu<?> baseMenu) {
     ...
     */
    protected void syncWithOtherContainer(BaseContainerMenu<?> otherContainer, ServerPlayerEntity player) {
    }
    ...
    public boolean canUse(PlayerEntity playerIn) {
        ...
        if (!this.tile.isRemoved()) {
            ...
            return world.canSetBlock(this.tile.getPos());
        }
    ...
    protected boolean mergeItemStackRefill(ItemStack stack, int startIndex, int endIndex, boolean useEndIndex) {
        ...
        if (stack.isStackable()) {
            while (stack.getCount() > 0 && (!useEndIndex && k < endIndex || useEndIndex && k >= startIndex)) {
                ...
                if (!itemstack1.isEmpty() && ItemStack.canCombine(stack, itemstack1) && this.canInsertIntoSlot(stack, slot)) {
    ...
    protected boolean mergeItemStackMove(ItemStack stack, int startIndex, int endIndex, boolean useEndIndex) {
        ...
        while (!useEndIndex && k < endIndex || useEndIndex && k >= startIndex) {
            ...
            if (itemstack1.isEmpty() && slot.canInsert(stack) && this.canInsertIntoSlot(stack, slot)) {
                ...
                slot.setStackNoCallbacks(stack2);
                slot.markDirty();
                flag1 = true;
